...
def main():
    ...
    parser.add_argument('--ext', help='File extension. default: %(default)s',metavar='str', default='.mha')
    parser.add_argument('--prefix', help='Prefix of the output filename. default: %(default)s',metavar='str', default='SE')
    parser.add_argument('--compress', help='Compress the output image. default: %(default)s', type=str, choices=['auto','true','false'],default='auto')
    parser.add_argument('--offset', help='Offset to the number. default: %(default)s', type=int, metavar='int', default=1)
    parser.add_argument('--logdir', help='Directory to store logs. default: %(default)s', metavar='str', default=None)
    parser.add_argument('--verbose', help='Verbosity. default: %(default)s', type=int,metavar='level', default=0)
    ...
    if args.logdir is not None:
        ...
        handler = FileHandler(logdir / '{}.log'.format(datetime.today().strftime("%y%m%d_%H%M%S")))
    ...
    all_files = [str(e) for e in tqdm.tqdm(root_dir.glob('**/*'), desc='list all files') if e.is_file()]
    
    key_tags = ['PatientID', 'SeriesInstanceUID', 'AcquisitionDate', 'AcquisitionTime', 'ImageOrientationPatient', 'ImagePositionPatient']
    dcm_files = []
    ...
    def sort_dicom(df):
        orientation = np.array(df['ImageOrientationPatient'].iloc[0]).reshape((2,3))
    ...
    for patient_id, df_patient in df.groupby('PatientID'):
        ...
        for series_id, df_series in df_patient.groupby('SeriesInstanceUID'):
            ...
            dts = df_series.apply(lambda row: DT(row.AcquisitionDate + row.AcquisitionTime), axis=1).tolist()
            if len(df_series) <= 2:
                times.append(dts[0])
            else:
                dts.sort()
        ...
        for series_id, df_series in df_patient.groupby('SeriesInstanceUID'):
            ...
            output_filename = out_dir / patient_id / (prefix + '{:d}'.format(series_id2series_number[series_id] + offset) + ext)
